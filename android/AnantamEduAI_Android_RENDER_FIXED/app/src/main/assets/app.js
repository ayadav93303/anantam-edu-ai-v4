  ...
  backendWarmPromise=(async()=>{
    ...
    for(let i=0;i<3;i++){
      try{
        ...
        if(r.ok && d.ok){
          if(d.configured===false) throw new Error("AI backend is running, but GEMINI_API_KEY is not configured on Render.");
          return d;
        }
        throw new Error(d.error||`AI backend health check failed (${r.status}).`);
      }catch(e){last=e; await new Promise(r=>setTimeout(r,1200*(i+1)));}
    }
    backendWarmPromise=null;
    throw last||new Error("AI backend is temporarily unavailable.");
  })();
...
async function apiRequest(path, options={}, cfg={}){
  ...
  const timeout=cfg.timeout ?? 140000;
  if(path!=="/health") await warmBackend();
  let lastErr=null;
...
function renderPracticeQuestions(questions, meta){
  ...
  safe.forEach((q,i)=>{
    ...
    let opts="";
    if(options.length){
      const correctLetter=String.fromCharCode(65+Number(q.answerIndex||0));
      opts=`<div class="answers">${options.map((o,j)=>`<button type="button" data-correct="${String(j===Number(q.answerIndex))}" onclick="answerPractice(this)">${esc(String.fromCharCode(65+j)+". "+o)}</button>`).join("")}</div><div class="practice-answer" hidden><b>Correct answer:</b> ${esc(correctLetter+". "+(options[Number(q.answerIndex||0)]||q.answer||""))}${q.explanation?`<br><b>Explanation:</b> ${esc(q.explanation)}`:""}</div>`;
    } else {
      opts=`<button type="button" class="show-answer" onclick="this.nextElementSibling.hidden=false">Show answer</button><div class="practice-answer" hidden><b>Answer:</b> ${esc(q.answer||"See solution")}${q.explanation?`<br><b>Explanation:</b> ${esc(q.explanation)}`:""}</div>`;
    }
